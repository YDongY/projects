...
def create_deployer_group():
    """
    添加一个名为的新组 deployers 并为其授予 sudo 权限，以便用户可以使用 root 权限执行进程
    """
...
def create_deployer_user():
    """
    用户组添加一个新用户，设置用于保存 SSH 密钥对的 SSH 目录，并授予组和用户访问该目录的权限
    """
...
def upload_keys():
    """
    将公钥和授权密钥复制到远程服务器新创建的 ssh-keys 目录下
    """
